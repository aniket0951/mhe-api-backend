...
from apps.lab_and_radiology_items.models import (
    HomeCollectionAppointment, PatientServiceAppointment)
...
from apps.url_shortner.utils import short_it
...
def get_report_info(hospital_code=None, specific_date=None):
    ...
    param['family_member_count'] = FamilyMember.objects.filter(
        is_visible=True, created_at__date=yesterday.date()).count()
    param['primary_user_count'] = Patient.objects.filter(
        mobile_verified=True, created_at__date=yesterday.date()).count()
    param['android_download'] = MobileDevice.objects.filter(
        platform='Android', participant__created_at__date=yesterday.date()).count()
    param['ios_download'] = MobileDevice.objects.filter(
        platform='iOS', participant__created_at__date=yesterday.date()).count()
    param['hv_count'] = Appointment.objects.filter(created_at__date=yesterday.date(
    ), booked_via_app=True, status=1, appointment_mode="HV", hospital__code=hospital_code).count()
    param['vc_count'] = Appointment.objects.filter(created_at__date=yesterday.date(
    ), booked_via_app=True, status=1, appointment_mode="VC", payment_status="success", hospital__code=hospital_code).count()
    param['hc_count'] = HealthPackageAppointment.objects.filter(created_at__date=yesterday.date(
    ), payment_id__status="success", appointment_status="Booked", hospital__code=hospital_code).count()
    param['hv_amount'] = Payment.objects.filter(created_at__date=yesterday.date(), status="success", appointment__isnull=False,
                                                appointment__appointment_mode="HV", location__code=hospital_code).aggregate(amount=Sum('amount'))['amount']
    param['vc_amount'] = Payment.objects.filter(created_at__date=yesterday.date(), status="success", appointment__isnull=False,
                                                appointment__appointment_mode="VC", location__code=hospital_code).aggregate(amount=Sum('amount'))['amount']
    param['ip_deposit_amount'] = Payment.objects.filter(created_at__date=yesterday.date(
    ),  payment_for_ip_deposit=True, status="success", location__code=hospital_code).aggregate(amount=Sum('amount'))['amount']
    param['hc_package_amount'] = Payment.objects.filter(created_at__date=yesterday.date(
    ), payment_for_health_package=True, status="success", location__code=hospital_code).aggregate(amount=Sum('amount'))['amount']
    param['op_outstanding_amount'] = Payment.objects.filter(created_at__date=yesterday.date(
    ), payment_for_op_billing=True, status="success", location__code=hospital_code).aggregate(amount=Sum('amount'))['amount']
    param['registration_amount'] = Payment.objects.filter(created_at__date=yesterday.date(
    ), payment_for_health_package=True, status="success", location__code=hospital_code).aggregate(amount=Sum('amount'))['amount']
    param['registered_patient_count'] = Payment.objects.filter(created_at__date=yesterday.date(
    ), payment_for_health_package=True, status="success", location__code=hospital_code).count()
    param['home_collection_count'] = HomeCollectionAppointment.objects.filter(
        created_at__date=yesterday.date(), hospital__code=hospital_code).count()
    param['home_service_count'] = PatientServiceAppointment.objects.filter(
        created_at__date=yesterday.date(), hospital__code=hospital_code).count()
    param['preferred_hospital_count'] = Patient.objects.filter(
        mobile_verified=True, created_at__date=yesterday.date(), favorite_hospital__code=hospital_code).count()
    param['ip_deposit_count'] = Payment.objects.filter(created_at__date=yesterday.date(
    ), payment_for_ip_deposit=True, status="success", location__code=hospital_code).count()
...
def assign_users(request_data, user_id):
    ...
    if request_data.get("family_member"):
        selected_patient = FamilyMember.objects.filter(
            id=request_data.get("family_member")).first()
        if not selected_patient:
...
def end_date_vaccination_date_comparision(booking_end_time, date_of_vaccination_date):
    ...
    date_of_vaccination_date_time = datetime.strptime(
        date_of_vaccination_date, '%Y-%m-%d')
    if end_date_time.date() != date_of_vaccination_date_time.date():
        raise ValidationError(
            'End date should be the same as the vaccination date.')
...
def short_url(complete_url):
    if not complete_url or len(complete_url) < 10:
        raise ValidationError("Invalid url")
    shorten_url = short_it(complete_url)
    return {
