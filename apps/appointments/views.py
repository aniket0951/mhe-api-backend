        ...
        if ManipalAdmin.objects.filter(id=self.request.user.id).exists():
            ...
            if is_upcoming:
                return super().get_queryset().filter(
                    Q(appointment_date__gte=datetime.now().date()) & Q(status=1) & ((Q(uhid__isnull=False) & Q(uhid=member_uhid)) | Q(family_member_id=family_member))).exclue(vc_appointment_status="4")
            return super().get_queryset().filter(
